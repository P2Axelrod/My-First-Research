...
class Dataset():
    ...
    def __init__(self, file, load=True, multiclass=True, normalize=False, istrain = False):
        self.file = file
        self._features = self._xyz = self._classes = self._names = self._waveform = None
    ...
    def load_data(self):
        ...
        if(self.istrain == False):
            self._classes = file_h.classification - 1 # only making train data
        else:
