...
class S3DISDataset(PointCloudDataset):
    ...
    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        ...
        if self.set == 'training':
            ...
            self.cloud_names = [f for i, f in enumerate(self.cloud_names)
                                if self.all_splits[i] == self.validation_split]
        # print("self.cloud_names = {}".format(self.cloud_names))
        ...
        if use_potentials:
            ...
            for i, tree in enumerate(self.pot_trees):
                ...
                # print("self.potentials= {}".format(len(self.potentials[-1])))
                ...
                print("self.min_potentials= {}".format(self.min_potentials))
    ...
    def potential_item(self, batch_i, debug_workers=False):
        ...
        while True:
            ...
            with self.worker_lock:
                ...
                # print("********************cloud_ind = {}***********************************".format(cloud_ind))
                # print("***********************point_ind = {}*******************************".format(point_ind))
...
# ******************************************************************************************************************************
            ...
            # print("input_points.shape = {}".format(input_points.shape))
            ...
            # Update batch size
            batch_n += n
            # print("int(self.batch_limit) = {}".format(int(self.batch_limit)))
            # print("batch_n = {}".format(batch_n))
        ...
        input_list = self.segmentation_inputs(stacked_points,
                                              ...
                                              labels,
                                              stack_lengths)
        # print("stacked_points.shape = {}".format(stacked_points.shape))
        # print("stacked_features.shape = {}".format(stacked_features.shape))
        # print("****input_list[1].shape = {}********".format(input_list[1].shape))
        t += [time.time()]

        # Add scale and rotation for testing
        input_list += [scales, rots, cloud_inds, point_inds, input_inds]
        # print("****input_list[26].shape = {}********".format(input_list[26]))
        if debug_workers:
            message = ''
            for wi in range(info.num_workers):
                if wi == wid:
    ...
    def prepare_S3DIS_ply(self):
        ...
        for cloud_name in self.cloud_names:
            ...
            print("room_folders = {}".format(room_folders))
    ...
    def load_subsampled_clouds(self):
        ...
        for i, file_path in enumerate(self.files):
            ...
            if exists(KDTree_file):
                ...
                with open(KDTree_file, 'rb') as f:
                    ...
                    # print("search_tree = {}".format(search_tree))
        ...
        # print("self.num_clouds = {}".format(self.num_clouds))
        # Only necessary for validation and test sets
            ...
            for i, file_path in enumerate(self.files):
                ...
                if exists(proj_file):
                    with open(proj_file, 'rb') as f:
                        ...
                        print("proj_inds.shape={}".format(proj_inds.shape))
                else:
                    data = read_ply(file_path)
                    points = np.vstack((data['x'], data['y'], data['z'])).T
                    labels = data['class']
                    ...
                    # print("proj_inds={}".format(proj_inds))
...
class S3DISCustomBatch:
    ...
    def __init__(self, input_list):
        ...
        ind += L
        self.one_hots = [torch.from_numpy(nparray) for nparray in input_list[ind:ind + L]]
        ind += L
    ...
    def pin_memory(self):
        ...
        self.one_hots = [in_tensor.pin_memory() for in_tensor in self.one_hots]
    ...
    def to(self, device):
        ...
        self.one_hots = [in_tensor.to(device) for in_tensor in self.one_hots]
