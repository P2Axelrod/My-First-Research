...
class PointCloudDataset(Dataset):
    ...
    def segmentation_inputs(self,
        ...
        one_hot_labels = F.one_hot(torch.from_numpy(labels), 13)
        # print("one_hot_labels.shape = {}".format(one_hot_labels.shape))
        One_Hot_Labels += [one_hot_labels.numpy().astype(np.int64)]
        for block_i, block in enumerate(arch):
            ...
            deform_layer = False
            if layer_blocks:
                # print("layer_blocks = {}".format(layer_blocks))
                # Convolutions are done in this layer, compute the neighbors with the good radius
                if np.any(['deformable' in blck for blck in layer_blocks]):
                    r = r_normal * self.config.deform_radius / self.config.conv_radius
                    deform_layer = True
                    ...
                    r = r_normal
                conv_i = batch_neighbors(stacked_points, stacked_points, stack_lengths, stack_lengths, r)
                # print("conv_i.shape = {}".format(conv_i.shape))
            ...
            if 'pool' in block or 'strided' in block:
                ...
                # Subsampled points
                pool_p, pool_b = batch_grid_subsampling(stacked_points, stack_lengths, sampleDl=dl)
                # pool_p是 subsampling的点[x,y,z]，pool_b是抽样的个数
                # print("pool_p = {}".format(pool_p))
                # print("pool_b = {}".format(pool_b))
            ...
            pool_i = self.big_neighborhood_filter(pool_i, len(input_points))
            # print("pool_i = {}".format(pool_i))
            # print("pool_i.shape = {}".format(pool_i.shape))
            ########################
            # one_hot of every layer
            ########################
            one_hot_labels = torch.cat((one_hot_labels, torch.zeros_like(one_hot_labels[:1, :])), 0)
            # print("one_hot_labels.shape = {}".format(one_hot_labels.shape))
            if len(pool_i) > 0:
                # print("我在这********************")
                # print("one_hot_labels.size() = {}".format(one_hot_labels.size()))
                # print("pool_i.shape = {}".format(pool_i.shape))
                one_hot_layer = one_hot_labels[torch.from_numpy(pool_i).long()]
                # print("one_hot_layer.shape = {}".format(one_hot_layer.shape))
                one_hot_labels = torch.max(one_hot_layer, 1)[0]
                # print("one_hot_labels.size() = {}".format(one_hot_labels.size()))
                One_Hot_Labels += [one_hot_labels.numpy().astype(np.int64)]
                # one_hot_labels = torch.cat((one_hot_labels, torch.zeros_like(one_hot_labels[:1, :])), 0)
                # print("one_hot_layer = {}".format(one_hot_labels))
            # else:
            #     print("我在这********************")
            #     print("one_hot_labels.size() = {}".format(one_hot_labels.size()))
            #     One_Hot_Labels += [np.array([]).astype(np.int64)]
            # print("One_Hot_Labels = {}".format(One_Hot_Labels))
            # print("q = {}".format(q))
            # # print("conv_i.shape = {}".format(conv_i.shape))
            # print("pool_i = {}".format(torch.from_numpy(pool_i)))
            # print("one_hot_labels.size() = {}".format(one_hot_labels.size()))
            if up_i.shape[0] > 0:
            ...
            # New points for next layer (为下一层抽样更新)
            stacked_points = pool_p
            stack_lengths = pool_b
            ...
            if 'global' in block or 'upsample' in block:
                break
        # print("stacked_features.shape = {}".format(stacked_features.shape))
        ###############
        # Return inputs
        ###############
        # list of network inputs
        li = input_points + input_neighbors + input_pools + input_upsamples + input_stack_lengths + One_Hot_Labels
        li += [stacked_features, labels]
