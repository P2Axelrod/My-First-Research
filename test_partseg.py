...
@hydra.main(config_path='config', config_name='test_partseg')
def main(args):
    ...
    print("args.input_dim = {}".format(args.input_dim))
    args.num_class = 50
    num_category = 16
    ...
    checkpoint = torch.load(hydra.utils.to_absolute_path(
    ...
    with torch.no_grad():
        ...
        for batch_id, (points, label, target) in tqdm(enumerate(testDataLoader), total=len(testDataLoader),
            ...
            vote_pool = torch.zeros(
                target.size()[0], target.size()[1], num_part).cuda()

            for _ in range(args.num_votes):
                # seg_pred, _ = classifier(
                #     points, to_categorical(label, num_category))
                seg_pred = classifier(torch.cat([points, to_categorical(
