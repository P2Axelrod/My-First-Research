...
def main(args):
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (points, label, target) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            ...
            # print(to_categorical(label, num_category).repeat(
            #     1, points.shape[1], 1).size())   #---> torch.Size([8, 1024, 16])
            # print(torch.cat([points, to_categorical(
            #     label, num_category).repeat(1, points.shape[1], 1)], -1).size())  # ---> torch.Size([8, 1024, 22])
